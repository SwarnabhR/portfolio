...
`design-system/` is **inspiration only** — it documents the Nyro Silvan Framer site aesthetic that informed the visual direction of this portfolio. This is _not_ a spec to replicate. The actual site belongs to S. Roy (a quant developer), so content, layout, and components should reflect that identity, not the original designer's.
